...
translate([5, 0,0])
open_box(width=10, depth=15, height=5, wall_thickness=1, chamfer=0);
...
module open_box_with_lip(width, depth, outer_height, inner_height, outer_wall_thickness, inner_wall_thickness, chamfer=0) {
    ...
    inner_trans_z = (outer_height - inner_height) /2  + eps;
    union() {
        open_box(width=width+outer_wall_offset, depth=depth+outer_wall_offset, height=outer_height, wall_thickness=outer_wall_thickness, chamfer=chamfer);
        translate([0,0, -1 *inner_trans_z])
        open_box(width=width + eps, depth=depth+ eps, height=inner_height, wall_thickness=inner_wall_thickness, chamfer=0, bottomless=true);
...
module open_box(width, depth, height, wall_thickness, chamfer=0, bottomless=false){
